...
@component('mail::button', ['url' => route('auth.activate', ['token' => $user->activation_token, 'email' => Crypt::encryptString($user->email)])])
Button Text
@endcomponent

Sebagai antisipasi jika tombol di atas tidak berfungsi, silahkan <i>copy &amp; paste</i> tautan berikut pada peramban Anda.

{{route('auth.activate', ['token' => $user->activation_token, 'email' => Crypt::encryptString($user->email)])}}
