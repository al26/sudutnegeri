                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if ($volunteer->status === 'accepted' || $volunteer->status === 'finished')
                                                ---
                                            @else
                                                <a href="{{route('volunteer.show', ['id' => encrypt($volunteer->id)])}}" class="btn btn-sm btn-primary" data-toggle="modal" data-target="#modal" data-backdrop="static" data-keyboard="false" data-modal='{"title":"Tinjau Aplikasi","yes":"Terima", "no":"Tolak", "lg":true, "yesUrl":"{{route('volunteer.accept', ['id' => $volunteer->id])}}", "noUrl":"{{route('volunteer.reject', ['id' => $volunteer->id])}}", "redirectAfter":"{{route('dashboard', ['menu' => 'sudut', 'section' => 'volunteer'])}}", "pjax-container":"#mr"}'><i class="fas fa-search"></i> Tinjau</a>
